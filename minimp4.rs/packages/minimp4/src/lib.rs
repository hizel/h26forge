...
impl<W: Write + Seek> Mp4Muxer<W> {
    ...
    pub fn init_video(&mut self, width: i32, height: i32, is_hevc: bool, enable_fragmentation: bool, track_name: &str) {
        self.str_buffer.push(CString::new(track_name).unwrap());
        unsafe {
            if self.muxer.is_null() {
                let self_ptr = self as *mut Self as *mut c_void;                
                self.muxer = MP4E_open(0, if enable_fragmentation { 1 } else { 0 }, self_ptr, Some(Self::write));
            }
            mp4_h26x_write_init(
                self.muxer_writer,
                self.muxer,
...
mod tests {
    ...
    fn test_muxer() {
        ...
        muxer.init_video(1280, 720, false, false, "test");
    ...
    fn test_mux_h264_audio() {
        ...
        mp4muxer.init_video(1280, 720, false, false, "h264 stream");
    ...
    fn test_mux_h265_audio() {
        ...
        mp4muxer.init_video(1280, 720, true, false, "h265 stream");
